...
#    AW = np.ones(T.shape)
    ...
    while test > 0.5 and iter < mxiter:
        ...
        #MB- AB[msk] = TB[msk] * KB[msk] / (aH[msk] + KB[msk])
        AB = TBKB / (aH + KB)
        ...
        #
        old_aH = np.copy(aH)
        #MB- aH[msk] = (0.5 * K1[msk] / AC[msk]) * ((TCO2[msk] - AC[msk]) + np.sqrt((TCO2[msk] - AC[msk]) * (TCO2[msk] - AC[msk]) + 4.0 * (AC[msk] * K2[msk] / K1[msk]) * (2.0 * TCO2[msk] - AC[msk])))
        temp = TCO2_AC*TCO2_AC + (AC * K2_K1x4) * (2.0 * TCO2 - AC)
        aH[msk] = (K2_2[msk] / AC[msk]) * (TCO2_AC[msk] + np.sqrt(temp[msk]))
