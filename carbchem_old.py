...
def itt_loop(test,iter,mxiter,TB,KB,aH,msk,AW,KW,AC,TALK,AB,K1,TCO2,K2,tol_swtch,tol,count):
    while  (test > 0.5 or iter >= mxiter):
      ...
      # and recalculate aH with the new As
        ...
        tol_swtch[msk] = ( np.abs( ( aH[msk] - old_aH[msk] )/old_aH[msk] ) > tol )
...
#       9 ;            saturation state: aragonite
    ...
    aH=itt_loop(test,iter,mxiter,TB,KB,aH,msk,AW,KW,AC,TALK,AB,K1,TCO2,K2,tol_swtch,tol,count)
    # note: moving the while loop into a function speed the script up hugely
...
#7.958 sec
#moving while loop into function spead code up to 1.725 sec
#92.856034    346.22870259
