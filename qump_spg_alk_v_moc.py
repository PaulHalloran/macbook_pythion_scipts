...
for i in range(qump_data[3,0,0,:].size):
    ax2.plot(qump_year,qump_data[6,0,:,i]-np.mean(qump_data[6,0,0:19,i]))

plt.show()

var=1
for i in range(qump_data[3,0,0,:].size):
    plt.scatter(qump_data[var,0,:,i]-np.mean(qump_data[var,0,0:19,i]),qump_data[6,0,:,i]-np.mean(qump_data[6,0,0:19,i]))
