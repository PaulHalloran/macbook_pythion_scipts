...
box_co2_flux2=np.copy(box_co2_flux)
box_years=input1[:,0]
...
shape_var=box_co2_flux.shape

for i in np.arange(np.size(filenamesb)):
    for j in np.arange(np.size(filenamesc)):
        for k in np.arange(shape_var[1]):
            box_co2_flux2[:,k,j,i]= scipy.signal.filtfilt(b, a, box_co2_flux[:,k,j,i])
...
for k in np.arange(np.size(dir_name2)):
    for i in range(np.size(no_filenames)):
        qump_data2[k,:,i]=scipy.signal.filtfilt(b, a, qump_data[k,:,i])
...
for i in range(1):
	...
	for k in np.append(np.arange(0,6),[11]):
		...
		ln_tmp=plt.plot(qump_year,(box_co2_flux2[:,i,k]/1.12e13)*1.0e15/12.0,label=plot_names[k])
		lns += ln_tmp
	...
	for k in np.append([0],np.arange(6,11)):
		ln_tmp=plt.plot(qump_year,(box_co2_flux2[:,i,k]/1.12e13)*1.0e15/12.0,marker=styles[k],markersize=4,label=plot_names[k])
		lns += ln_tmp
		#this should plot out the variously filtered (at input to box model) box model results,
		#for the QUMP ensemble member specified by i. - should be able to spot which fail to match QUMP flux
	...
	lns=plt.plot(qump_year,qump_data2[2,:,i]-np.mean(qump_data[2,:,i]),c='b',linewidth=2,label='QUMP flux')
	for k in np.arange(12,19):
		ln_tmp=plt.plot(qump_year,((box_co2_flux2[:,i,k]/1.12e13)*1.0e15/12.0)-np.mean((box_co2_flux[:,i,k]/1.12e13)*1.0e15/12.0),marker=styles[k],markersize=4,label=plot_names[k])
		lns += ln_tmp
		#this should plot out the variously filtered (at input to box model) box model results,
		#for the QUMP ensemble member specified by i. - should be able to spot which fail to match QUMP flux
	labs = [l.get_label() for l in lns]
...
for i,k in enumerate(np.append([0],np.arange(6,11))):
    ...
    for j in np.arange(np.size(filenamesb)):
        ax1.scatter(qump_data2[0,:,:],(box_co2_flux2[:,:,k,j]/1.12e13)*1.0e15/12.0, s=sym_size_var, facecolor='1.0',color=colours[j], lw = line_width_var)
...
for i in (np.arange(6)+start):
	...
	for k in np.array([0,19,20,21,22]):
		#ln_tmp=plt.plot(qump_year,(box_co2_flux[:,i,k]/1.12e13)*1.0e15/12.0,marker=styles[k],markersize=4,label=plot_names[k])
		ln_tmp=plt.plot(qump_year,(box_co2_flux2[:,i,k,0]/1.12e13)*1.0e15/12.0,label=plot_names[k])
		lns += ln_tmp
		#this should plot out the variously filtered (at input to box model) box model results,
		#for the QUMP ensemble member specified by i. - should be able to spot which fail to match QUMP flux
