...
def pp_file_volume_total(filename,stash,lon1,lat1,lon2,lat2):
    stash_tmp=[]
    midpoint_depth=[]
    bottom_depth=[]
    #
    ...
    cube[0].data += 1.0
    #
    region = iris.Constraint(longitude=lambda v: lon1 <= v <= lon2,latitude=lambda v: lat1 <= v <= lat2)
    #
...
ocean_volume_m3=pp_file_volume_total(filename,stash,lon1,lat1,lon2,lat2)
