...
for k,model_var_cnt in enumerate(model_vars):
	for i in range(np.size(no_filenames)):
		...
		out[0,:]=qump_year
		out[1:,:]=qump_low_pass
		np.savetxt(dir_name2+'low_pass_2/qump_data_'+file_names[i]+'_'+model_var_cnt+'.txt', np.transpose(out), delimiter=',')
		out[1:,:]=qump_band_pass
                #print np.mean(qump_band_pass)
		np.savetxt(dir_name2+'/band_pass_2/qump_data_'+file_names[i]+'_'+model_var_cnt+'.txt', np.transpose(out), delimiter=',')
		out[1:,:]=qump_band_pass_around_zero
		np.savetxt(dir_name2+'/band_pass_2_around_zero/qump_data_'+file_names[i]+'_'+model_var_cnt+'.txt', np.transpose(out), delimiter=',')
...
for k,model_var_cnt in enumerate(model_vars):
	for i in range(np.size(no_filenames)):
		...
		out[0,:]=qump_year
		out[1:,:]=qump_low_pass
		np.savetxt(dir_name2+'low_pass_3/qump_data_'+file_names[i]+'_'+model_var_cnt+'.txt', np.transpose(out), delimiter=',')
		out[1:,:]=qump_band_pass
                #print np.mean(qump_band_pass)
		np.savetxt(dir_name2+'band_pass_3/qump_data_'+file_names[i]+'_'+model_var_cnt+'.txt', np.transpose(out), delimiter=',')
		out[1:,:]=qump_band_pass_around_zero
		np.savetxt(dir_name2+'band_pass_3_around_zero/qump_data_'+file_names[i]+'_'+model_var_cnt+'.txt', np.transpose(out), delimiter=',')
